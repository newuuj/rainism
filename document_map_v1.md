...
> 2026-08-17 갱신 · **문서가 헷갈리면 여기부터 본다**
>
...
| 탭 구조 | **2탭: 오늘(코어) · 기록**(비공개 캘린더). 날씨는 **오늘 탭 안 카드** *(2026-08-17 통합)*. 소셜/피드 없음 |
...
| 재방문 장치 | **아침 웹푸시 1회 = 코어**(2026-08-16 승격). 위젯은 여전히 🚫(네이티브 필요) |
| 북극성 | **알림 도달일 결정률** — 알림 보낸 날 중 코디를 정한 날의 비율 (2026-08-16 교체, 이전 = D1 아침 리텐션 → 보조로 강등) |
| 취향 질문 | **1개뿐 — "어떤 게 제일 마음에 들어요?" · 사진 12장(컨셉 4종×3장) 중 원하는 만큼** (2026-08-17, 개수 제한 없음). 색·TPO 문항은 2026-08-16 폐기 |
| 프로토타입 UI | **분홍 테마 + 로고 아트 스플래시** (2026-08-17). 토큰 = `09_design_system §4` |
...
├── prototype/      ← 작동하는 프로토타입 + 실사진 124장 (코디 라이브러리는 Supabase에 110장)
├── seed_photos/    ← 라이브러리 시드 후보 원본. 🚫 저장소에 안 올린다(.gitignore) — 각자 컴퓨터에만
...
| `tech/rainism_outfit_rules_v2_allseason_women.csv` | 날씨 하드필터 규칙 원본. **v2(2026-08-16): 전 계절 8밴드 + 눈·빙판 `[6]`** (v1은 여름 4밴드) |
...
5. **검증 계획서** *(2026-08-07 신규 · 2026-08-16 갱신)* — 몇 명에게 몇 주 돌리고, **결정률**이 몇 %면 계속 가고 몇 %면 접는가. *(착용률은 2026-08-17 폐기 — 「입었어요」를 더 이상 묻지 않는다. 대체 눈금 미정 = PRD 부록 B #14)* PRD §4가 이걸 🔬로 열어뒀다. **유저가 정해야 한다.** ※지표가 D1 리텐션 → **알림 도달일 결정률**로 바뀌었다(2026-08-16).
6. **알림 정책서** *(2026-08-16 신규)* — 아침 알림이 코어가 되면서 필요해진 문서. **발송 시각·문구 규칙·오알림 판정 기준(강수확률 몇 % 이상)·끄기 경로.** 임계값은 근거가 없어 비어 있다 → **유저가 정해야 한다.**
